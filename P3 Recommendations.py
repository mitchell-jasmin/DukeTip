...
movieNames = np.loadtxt('./ml-100k/u.item', delimiter='|',dtype={'names': ('id', 'name'),
                        'formats': (np.int, 'S128')})  # replace 0 with the correct code to load the movie names
...
movieData = np.loadtxt('./ml-100k/u.data', dtype={'names': ('user', 'movie', 'rating'),
                       'formats': (np.int, np.int, np.int)})  # replace 0 with the correct code to load the movie data
